...
|------|------|--------|-------------|--------|
| param | `Object` | - | 否 | 参数列表 |
| param.renderType | `THING.HeatmapRenderType` | `THING.HeatmapRenderType.Plane` | 是 | 是平面的热力图还是3d的热力图THING.HeatmapRenderType.Plane代表平面热力图 THING.HeatmapRenderType.Mesh代表3d热力图 |
| param.extent | `Number` | - | 是 | 热力图展示的范围 例如{minX:116,minY:39,maxX:117,maxY:40} 数值代表的是经纬度 |
...
| param.data | `Array` | - | 否 | 创建热力图使用的数据源 |
| param.gradient | `Object` | `false` | 是 | 渲染热力图使用的色带 |
| param.mask | `Array` | - | 是 | 渲染热力图使用的遮罩可传入geojson格式的多边形坐标 |
| param.mosaic | `Object` | - | 是 | 热力图是否使用马赛克效果 格式为{enable:true,size:2} enable代表是否使用马赛克 size代表马赛克尺寸 |
| param.radius | `Number` | `5` | 是 | 计算热力图使用的半径 |
| param.extrudeHeight | `Number` | `5` | 是 | 3d热力图模式下 热力图热度最高处的拔起高度 单位米 |
| param.transparent | `Boolean` | `false` | 是 | 无热度的区域是否透明 |
| param.offsetHeight | `Number` | `0` | 是 | 热力图离地高度 |
