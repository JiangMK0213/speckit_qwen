...
| 名称 | 类型 | 描述 |
|------|------|-------------|
| `from` | `number, object` | 起始值。 |
| `to` | `number, object` | 目标值。 |
| `duration` | `number` | 插值时间（毫秒）。 |
| `delayTime` | `number` | 延迟时间（毫秒）。 |
| `options` | `object` | 可选参数。 |
| `options.noInterp` | `boolean` | 是否直接设置，不进行插值。 |
...
```javascript
const box = new THING.Box();THING.App.current.tweenManager.lerpTo({ opacity: 1}, { opacity: 0.2}, 2000).onUpdate(ev => {	box.style.opacity = ev.value.opacity;});
```
