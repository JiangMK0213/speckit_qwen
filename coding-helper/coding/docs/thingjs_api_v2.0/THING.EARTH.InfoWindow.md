...
**继承:** [THING.BaseObject](./THING.BaseObject.md)
...
| param.type | `String` | `Standard` | 是 | 信息框的类型，支持InfoWindowType.Standard(标准)和InfoWindowType.Custom(自定义)两种类型，type为InfoWindowType.Standard 会读取fieldData中的信息设置弹窗内容 type为InfoWindowType.Custom 会根据customHtml设置弹窗内容 |
| param.customHtml | `String` | - | 是 | 自定义信息框的html文本,仅在type为custom时生效 |
| param.displayMode | `String` | `DisplayMode.Always` | 是 | 信息框显示方式，包括DisplayMode.MouseEnter(悬浮显示)、DisplayMode.Click(点击显示)、DisplayMode.Always(一直显示) DisplayMode.None(不显示)四种类型 |
| param.title | `String` | `详细信息` | 是 | 信息框的标题 |
| param.style | `String` | `Default` | 是 | 信息框的样式，包括InfoWindowStyle.Default、InfoWindowStyle.Blue和InfoWindowStyle.White三种类型 |
| param.localPosition | `Array.<Number>` | `[0,0,0]` | 是 | 信息框的偏移量 [x,y,z]分别代表三个轴偏移量 上下为y轴 左右为x轴 前后为z轴 |
| param.pivot | `Array.<Number>` | `[0.5,0]` | 是 | 信息框的轴心，以百分比表示界面轴心位置 [0,0] 代表界面左上；[1,1] 代表界面右下 默认[0.5,0] 数组每个值可以大于1用于控制像素级别的offset |
| param.fieldData | `Array` | - | 是 | 信息框中显示的字段、字段别名、超链接和弹出框方式四种数据 |
...
**数据类型:** `THING.EARTH.InfoWindowType`

**默认值:** `THING.EARTH.InfoWindowType.Standard`

**重写:** THING.BaseObject#type

**描述:** 信息框的类型，有 Standard(标准) 和 Custom(自定义) 两种类型，</br>type为Standard会读取fieldData中的信息设置弹窗内容 type为Custom 会根据customHtml设置弹窗内容
...
**描述:** 自定义信息框的html文本，仅当 type='custom'时生效
...
**数据类型:** `THING.EARTH.InfoWindowStyle`

**默认值:** `Default`

**描述:** 信息框的样式，有Default(默认)、Blue(蓝色) 和 White(白色)
...
**数据类型:** `THING.EARTH.DisplayMode`

**描述:** 信息框的显示方式，有包括DisplayMode.MouseEnter(悬浮显示)、DisplayMode.Click(点击显示)、DisplayMode.Always(一直显示) DisplayMode.None(不显示)四种类型
...
**继承:** THING.BaseObject#name

**重写:** THING.BaseObject#name
...
**重写:** THING.BaseObject#destroyable
...
**重写:** THING.BaseObject#relationships
...
**重写:** THING.BaseObject#components
...
**重写:** THING.BaseObject#hasAttribute
...
**重写:** THING.BaseObject#getAttribute
...
**重写:** THING.BaseObject#setAttribute
...
**重写:** THING.BaseObject#setAttributes
...
**重写:** THING.BaseObject#pauseEvent
...
**重写:** THING.BaseObject#resumeEvent
...
**继承:** THING.BaseObject#add

**重写:** THING.BaseObject#add
...
**重写:** THING.BaseObject#queryByName
...
**重写:** THING.BaseObject#queryByTags
...
**重写:** THING.BaseObject#queryByUUID
...
**重写:** THING.BaseObject#queryByType
...
**重写:** THING.BaseObject#queryByDTType
...
**重写:** THING.BaseObject#queryByUserData
...
**重写:** THING.BaseObject#queryByRegExp
...
**重写:** THING.BaseObject#addComponent
...
**重写:** THING.BaseObject#removeComponent
...
**重写:** THING.BaseObject#removeAllComponents
...
**重写:** THING.BaseObject#traverseComponentByType
...
**重写:** THING.BaseObject#getComponentByName
...
**重写:** THING.BaseObject#getComponentByType
...
**重写:** THING.BaseObject#getComponentsByType
...
**重写:** THING.BaseObject#getAllComponents
...
**重写:** THING.BaseObject#hasComponent
