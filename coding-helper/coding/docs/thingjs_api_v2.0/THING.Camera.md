...
**描述:** 相机对象的构造方法 我们一般不会调用Camera的构造方法,因为new App时会默认创建Camera,直接通过app.camera可获取相机对象
...
**数据类型:** `THING.CameraEffectComponent`
...
**数据类型:** `THING.CameraPostEffectComponent`
...
**数据类型:** `THING.CameraControlComponent`
...
**描述:** 启用/禁用 视口渲染模式(即指定视口位置和尺寸,仅在该范围内进行渲染)
...
**数据类型:** `THING.RenderTexture`

**描述:** 设置/获取 RenderTexture
...
| 名称 | 类型 | 描述 |
|------|------|-------------|
| `object` | `Object3D` | the object which will be checked |
...
| 名称 | 类型 | 描述 |
|------|------|-------------|
| `x` | `number, Array.<number>` | 世界坐标 x 或包含 x,y,z 的数组 |
| `y` | `number` | 世界坐标 y |
| `z` | `number` | 世界坐标 z |
| `target` | `Array.<number>` | 用于存储结果的数组 |
...
| 名称 | 类型 | 描述 |
|------|------|-------------|
| `x` | `number, Array.<number>` | 屏幕坐标 x 或包含 x,y 的数组 |
| `y` | `number` | 屏幕坐标 y |
| `target` | `Array.<number>` | 用于存储结果的数组 |
...
- **类型:** `Promise.<THING.RenderTexture>`
- **描述:** Promise，解析为 RenderTexture
...
**描述:** 异步：对当前场景截图 并以 UInt8Array 的方式输出。
...
**描述:** 异步：对当前场景截图 并输出到 img 对象（base64 字符串作为 src）。
...
**描述:** 对当前场景截图 并输出到文件 调用后浏览器会自动下载该文件
...
| 名称 | 类型 | 描述 |
|------|------|-------------|
| `value` | `ProjectionType` | 投影类型 |
| `duration` | `number` | 投影类型切换的时间 |
...
| 名称 | 类型 | 描述 |
|------|------|-------------|
| `target` | `THING.Object3D` | 目标物体，如果不提供，则使用当前场景的物体或根物体。 |
| `value` | `ViewMode` | 视图模式值。 |
| `duration` | `number` | 投影矩阵过渡时间（毫秒）。 |
...
| 名称 | 类型 | 描述 |
|------|------|-------------|
| `region` | `Array.<number>` | 屏幕指定范围(从左上角计算,数组每个元素的单位是像素) |
| `object` | `THING.Object3D` | - |
| `included` | `boolean` | It fully includes the default false |
...
| `x` | `number` | 屏幕坐标x |
| `y` | `number` | 屏幕坐标y |
| `root` | `THING.BaseObject` | 拾取的根节点 默认是app.root |
...
| `x` | `number` | 屏幕坐标x |
| `y` | `number` | 屏幕坐标y |
| `isVertical` | `boolean` | true代表使用水平面和垂直面进行拾取,false代表仅适用水平面进行拾取 |
