...
**继承:** [THING.EARTH.Layer](./THING.EARTH.Layer.md)
...
|------|------|--------|-------------|--------|
| param | `Object` | - | 否 | - |
| param.url | `String` | - | 否 | 资源路径 支持标准的3dtiles服务 |
| param.offsetHeight | `Number` | `0` | 是 | 离地高度 |
| param.centerCoordinates | `Array` | - | 是 | 中心点经纬度(不传默认使用数据自己的) |
| param.skipLevelOfDetail | `Boolean` | `true` | 是 | 如果为true，渲染器会跳过一些中间层级，直接从粗略层级跳到详细层级，如果为false，渲染器会按照层级顺序依次加载和显示细节数据 |
| param.immediatelyLoadDesiredLevelOfDetail | `Boolean` | `false` | 是 | 用于控制是否立即加载所需的细节层级 |
| param.maximumScreenSpaceError | `Number` | `16` | 是 | 屏幕上允许的最大误差（以像素为单位） |
| param.baseScreenSpaceError | `Number` | `1024` | 是 | 初始屏幕空间误差值（以像素为单位） |
| param.cacheBytes | `Number` | `536870912` | 是 | 用于缓存瓦片的GPU显存量（以字节为单位）。该内存使用量是根据已加载瓦片的几何数据、纹理和批量表纹理估算得出的。</br>
      系统会通过卸载不可见瓦片来确保内存用量不超过此限制。若降低该数值导致需要卸载瓦片，这些瓦片将在下一帧被卸载。 |
| param.maximumCacheOverflowBytes | `Number` | `536870912` | 是 | 用于缓存瓦片的最大额外GPU内存量（以字节为单位）。</br>
      如果当前视图所需的瓦片大小（maximumScreenSpaceError）超过了cacheBytes加上maximumCacheOverflowBytes的总和，系统将调整Cesium3DTileset#memoryAdjustedScreenSpaceError，直到满足调整后屏幕空间误差所需的瓦片内存使用量小于cacheBytes加上maximumCacheOverflowBytes的总和。 |
| param.loadSiblings | `Boolean` | `false` | 是 | 用于控制在加载一个瓦片（tile）时是否同时加载其兄弟瓦片（siblings）。兄弟瓦片是指与当前瓦片同一级别（LOD）的其他瓦片。 |
...
**描述:** 如果为false，渲染器会按照层级顺序依次加载和显示细节数据。如果为true，渲染器会跳过一些中间层级，直接从粗略层级跳到详细层级。
...
**描述:** 最大屏幕空间误差用于驱动细节层次的优化,需要进行调整以达到适当的平衡。较高的值提供更好的性能，但视觉质量较低。
...
**描述:** 屏幕空间误差表示在当前视图下，一个瓦片的几何误差在屏幕上的投影大小。通过调整这个误差值，可以决定加载哪个细节层级的瓦片。</br>较大的误差值会减少细节层级的加载，可能会导致显示精度下降</br>较小的误差值会增加细节层级的加载，可能会导致性能下降，特别是在低性能设备上。</br>skipLevelOfDetail为true时生效
...
**描述:** 如果设置为false，则只会加载当前瓦片，不会同时加载其兄弟瓦片。如果设置为 true，在加载一个瓦片时会同时加载其兄弟瓦片。</br>注意：设置为true时，可以减少视图在不同细节层级之间的频繁跳跃，提供更平滑的用户体验。</br>但是会增加带宽和内存的使用，因为需要加载更多的瓦片，在低性能设备上，可能会导致加载时间变长，从而影响渲染性能。</br>skipLevelOfDetail为true时生效
...
**描述:** 是否立即加载所需的细节层级（Level of Detail，LOD)</br>如果设置为true,浏览时，会在当前视图范围内立即加载所需的最高细节层级，从而提高显示效果，但可能会增加加载时间和资源消耗</br>如果设置为false,浏览时会逐步加载所需的细节层级，这可以减少瞬时加载压力，但会导致细节逐步显示</br>skipLevelOfDetail为true时生效
...
**重写:** THING.EARTH.Layer#offsetHeight
...
**重写:** THING.EARTH.Layer#visibleLevel
...
**重写:** THING.EARTH.Layer#loadStatus
...
**重写:** THING.EARTH.Layer#isDynamicLoad
...
**重写:** THING.EARTH.Layer#layerMask
...
**重写:** THING.EARTH.Layer#frustumCulled
...
**重写:** THING.EARTH.Layer#renderOrder
...
**重写:** THING.EARTH.Layer#alwaysOnTop
...
**重写:** THING.EARTH.Layer#boundingBox
...
**重写:** THING.EARTH.Layer#orientedBox
...
**重写:** THING.EARTH.Layer#localPosition
...
**重写:** THING.EARTH.Layer#localAngles
...
**重写:** THING.EARTH.Layer#localRotation
...
**重写:** THING.EARTH.Layer#localQuaternion
...
**重写:** THING.EARTH.Layer#localScale
...
**重写:** THING.EARTH.Layer#quaternion
...
**重写:** THING.EARTH.Layer#ignoreParentTransform
...
**重写:** THING.EARTH.Layer#destroyable
...
**重写:** THING.EARTH.Layer#queryable
...
**重写:** THING.EARTH.Layer#relationships
...
**重写:** THING.EARTH.Layer#components
...
**描述:** 根据featurId获取顶点数据 这部分代码参考自Cesium的pickObject文件
...
**重写:** THING.EARTH.Layer#startScanning
...
**重写:** THING.EARTH.Layer#stopScanning
...
**重写:** THING.EARTH.Layer#pauseScanning
...
**重写:** THING.EARTH.Layer#resumeScanning
...
**重写:** THING.EARTH.Layer#destroyScanning
...
**重写:** THING.EARTH.Layer#getVisible
...
**重写:** THING.EARTH.Layer#setVisible
...
**重写:** THING.EARTH.Layer#setPickable
...
**重写:** THING.EARTH.Layer#getFrustumCulled
...
**重写:** THING.EARTH.Layer#setFrustumCulled
...
**重写:** THING.EARTH.Layer#distanceTo
...
**重写:** THING.EARTH.Layer#promoteNode
...
**重写:** THING.EARTH.Layer#waitForComplete
...
**重写:** THING.EARTH.Layer#loadResource
...
**重写:** THING.EARTH.Layer#unloadResource
...
**重写:** THING.EARTH.Layer#getRenderOrder
...
**重写:** THING.EARTH.Layer#setRenderOrder
...
**重写:** THING.EARTH.Layer#makeInstancedDrawing
...
**重写:** THING.EARTH.Layer#enableInstancing
...
**重写:** THING.EARTH.Layer#disableInstancing
...
**重写:** THING.EARTH.Layer#setInstancing
...
**重写:** THING.EARTH.Layer#getInstancing
...
**重写:** THING.EARTH.Layer#setInstancingColor
...
**重写:** THING.EARTH.Layer#setInstancingClearOutColor
...
**重写:** THING.EARTH.Layer#getInstancingClearOutColor
...
**重写:** THING.EARTH.Layer#setInstancingDiffuseUVTransform
...
**重写:** THING.EARTH.Layer#getInstancingDiffuseUVTransform
...
**重写:** THING.EARTH.Layer#setInstancingVisible
...
**重写:** THING.EARTH.Layer#getInstancingVisible
...
**重写:** THING.EARTH.Layer#getInstancedCount
...
**重写:** THING.EARTH.Layer#setInstancingMatrix
...
**重写:** THING.EARTH.Layer#getInstancingMatrix
...
**重写:** THING.EARTH.Layer#setInstancingAttribute
...
**重写:** THING.EARTH.Layer#getInstancingAttribute
...
**重写:** THING.EARTH.Layer#stopMoving
...
**重写:** THING.EARTH.Layer#pauseMoving
...
**重写:** THING.EARTH.Layer#resumeMoving
...
**重写:** THING.EARTH.Layer#movePathAsync
...
**重写:** THING.EARTH.Layer#stopScaling
...
**重写:** THING.EARTH.Layer#pauseScaling
...
**重写:** THING.EARTH.Layer#resumeScaling
...
**重写:** THING.EARTH.Layer#stopRotating
...
**重写:** THING.EARTH.Layer#pauseRotating
...
**重写:** THING.EARTH.Layer#resumeRotating
...
**重写:** THING.EARTH.Layer#stopFading
...
**重写:** THING.EARTH.Layer#pauseFading
...
**重写:** THING.EARTH.Layer#resumeFading
...
**重写:** THING.EARTH.Layer#fadeInAsync
...
**重写:** THING.EARTH.Layer#fadeOutAsync
...
**重写:** THING.EARTH.Layer#stopFlying
...
**重写:** THING.EARTH.Layer#pauseFlying
...
**重写:** THING.EARTH.Layer#resumeFlying
...
**重写:** THING.EARTH.Layer#stopUVTransform
...
**重写:** THING.EARTH.Layer#pauseUVTransform
...
**重写:** THING.EARTH.Layer#resumeUVTransform
...
**重写:** THING.EARTH.Layer#uvTransformTo
...
**重写:** THING.EARTH.Layer#rotateOnAxis
...
**重写:** THING.EARTH.Layer#translateX
...
**重写:** THING.EARTH.Layer#translateY
...
**重写:** THING.EARTH.Layer#translateZ
...
**重写:** THING.EARTH.Layer#translate
...
**重写:** THING.EARTH.Layer#worldToSelf
...
**重写:** THING.EARTH.Layer#selfToWorld
...
**重写:** THING.EARTH.Layer#localToWorld
...
**重写:** THING.EARTH.Layer#worldToLocal
...
**重写:** THING.EARTH.Layer#hasAttribute
...
**重写:** THING.EARTH.Layer#getAttribute
...
**重写:** THING.EARTH.Layer#setAttribute
...
**重写:** THING.EARTH.Layer#setAttributes
...
**重写:** THING.EARTH.Layer#pauseEvent
...
**重写:** THING.EARTH.Layer#resumeEvent
...
**重写:** THING.EARTH.Layer#queryByName
...
**重写:** THING.EARTH.Layer#queryById
...
**重写:** THING.EARTH.Layer#queryByTags
...
**重写:** THING.EARTH.Layer#queryByUUID
...
**重写:** THING.EARTH.Layer#queryByType
...
**重写:** THING.EARTH.Layer#queryByDTType
...
**重写:** THING.EARTH.Layer#queryByUserData
...
**重写:** THING.EARTH.Layer#queryByRegExp
...
**重写:** THING.EARTH.Layer#addComponent
...
**重写:** THING.EARTH.Layer#removeComponent
...
**重写:** THING.EARTH.Layer#removeAllComponents
...
**重写:** THING.EARTH.Layer#traverseComponentByType
...
**重写:** THING.EARTH.Layer#getComponentByName
...
**重写:** THING.EARTH.Layer#getComponentByType
...
**重写:** THING.EARTH.Layer#getComponentsByType
...
**重写:** THING.EARTH.Layer#getAllComponents
...
**重写:** THING.EARTH.Layer#hasComponent
