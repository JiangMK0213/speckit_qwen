...
| param.url | `string` | - | 否 | 地面图片的URL |
| param.maskUrl | `string` | - | 否 | 遮罩图片的URL |
| param.color | `string` | - | 否 | 地面颜色 |
| param.flowColor | `string` | - | 否 | 流动颜色 |
...
| 名称 | 类型 | 描述 |
|------|------|-------------|
| `target` | `Object3D` | 根据目标的边界框计算和更新特效地面的位置和缩放。 |
