...
| param.url | `String` | - | 否 | url 瓦片服务的url zxy格式 例如'https://webst01.is.autonavi.com/appmaptile?style=6&x={x}&y={y}&z={z}' |
| param.style | `THING.EARTH.TileLayerStyle` | - | 是 | style 渲染样式 |
| param.tilingSchemeType | `THING.EARTH.TilingSchemeType` | `THING.EARTH.TilingSchemeType.WebMercator` | 是 | 瓦片切片策略 支持WebMercator,Baidu,Geographic,GB四种 |
| param.maximumLevel | `Number` | - | 是 | 瓦片图地图服务最大级别 如不设置默认是无限制 超过该级别的瓦片将不被请求 |
...
**数据类型:** `THING.EARTH.TilingSchemeType`

**默认值:** `THING.EARTH.TilingSchemeType.WebMercator`

**描述:** 瓦片切片策略 支持WebMercator,Baidu,Geographic,GB四种
...
**描述:** 瓦片服务最小级别 缩小后到这个级别之下的层级，仍然会请求该级别瓦片，所以有可能会崩溃，此参数需要慎用
