...
|------|------|--------|-------------|--------|
| param | `object` | - | 否 | 初始参数。 |
| param.url | `string` | `''` | 是 | WebView 的 URL。 |
| param.domWidth | `number` | `1920` | 是 | WebView 的 DOM 宽度（像素）。 |
| param.domHeight | `number` | `1080` | 是 | WebView 的 DOM 高度（像素）。 |
| param.interactive | `boolean` | `false` | 是 | 是否启用交互功能。 |
| param.renderType | `RenderType` | `THING.RenderType.Plane` | 是 | 渲染类型。 |
| param.domScale | `number, Array.<number>` | - | 是 | DOM 缩放比例。可以是单个数值或长度为2的数组。 |
