...
**描述:** 提供处理对象的空间关系的能力。空间是具有一定体积的三维对象，可进行空间计算。</br>空间可以包含子空间和实体，实体也可以包含空间。空间关系可以分为包含、相交和相离。</br>空间计算时是以物体自身包围盒进行计算

**参数:**

| 名称 | 类型 | 默认值 | 可选 | 描述 |
|------|------|--------|-------------|--------|
| param | `object` | - | 否 | The initial parameters. 除Object3D构造方法中的参数之外,还可以传size |
| param.size | `Array.<number>` | - | 否 | 三个方向的尺寸 |
...
| 名称 | 类型 | 描述 |
|------|------|-------------|
| `obj` | `object` | 当前物体是否包含该参数传入的物体 |
| `recursive` | `boolean` | 空间计算是否包含obj的子对象 |
...
**描述:** 判读相离关系 相离代表两个物体之间没有交集

#### 参数

| 名称 | 类型 | 描述 |
|------|------|-------------|
| `obj` | `object` | 当前物体是否与该参数传入的物体不相交 |
| `recursive` | `boolean` | 空间计算是否包含obj的子对象 |
