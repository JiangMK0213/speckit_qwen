...
**继承:** [THING.Streamline](./THING.Streamline.md)
...
|------|------|--------|-------------|--------|
| param | `Object` | - | 否 | 参数列表 |
| param.extent | `Object` | - | 否 | 场数据范围 格式参考extent属性的接口文档 值代表经纬度 |
| param.data | `Object` | - | 是 | 计算使用的场数据 格式参考data属性的接口文档 场数据里的位置都是代表经纬度 |
| param.url | `String` | - | 是 | 计算使用的场数据的路径 data与url必须有一个不为空 |
| param.numLines | `Number` | `5000` | 是 | 流线个数 |
| param.gradient | `Object` | `{0: "#0000ff",0.33: "#00ff00",0.66: "#ffff00",1.0: "#ff0000"}` | 是 | 渐变色 |
| param.speed | `Number` | `1` | 是 | 流线速度 |
| param.maxAge | `Number` | `100` | 是 | 流线生命周期 每一个流线会在maxAge帧后消失 |
| param.renderType | `StreamlineRenderType` | `THING.StreamlineRenderType.Streamline` | 是 | 流线类型 |
| param.useDataColor | `Boolean` | `true` | 是 | renderType为Line时 是否使用该位置的标量值通过色带对线进行颜色渲染 |
| param.imageUrl | `String` | - | 是 | renderType为Line时 贴图的路径 |
...
**数据类型:** `StreamlineRenderType`

**默认值:** `THING.StreamlineRenderType.Streamline`

**继承:** THING.Streamline#renderType

**重写:** THING.Streamline#renderType

**描述:** 渲染类型 默认使用流线(Streamline)渲染 也支持普通线(Line)
...
**继承:** THING.Streamline#maxAge

**重写:** THING.Streamline#maxAge

**描述:** renderType为Streamline时 代表每一条流线的最大生命周期 maxAge越大流线存在的时间越久(流线会在maxAge帧后消失)
...
**默认值:** `{ 0: "#0000ff",0.33: "#00ff00",0.66: "#ffff00",1.0: "#ff0000"}`

**继承:** THING.Streamline#gradient

**重写:** THING.Streamline#gradient
...
**继承:** THING.Streamline#numLines

**重写:** THING.Streamline#numLines
...
**继承:** THING.Streamline#data

**重写:** THING.Streamline#data

**描述:** 计算流线使用的场数据 如果是规则格网的场数据,可以支持的格式为格式为:{u:[],v:[],w,[],dimX:x,dimY:y,dimZ:z} 基于次数据配合extent 可计算出规则格网每个格网的坐标值和对于的uvw值也可支持非规则格网数据 直接使用[x,y,z,u,v,w]的二维数组
...
**继承:** THING.Streamline#scalarData

**重写:** THING.Streamline#scalarData

**描述:** 场数据的标量形式,即将u,v,w通过Math.sqrt(u*u+v*v+w*w)计算后得到的数值
...
**继承:** THING.Streamline#minValue

**重写:** THING.Streamline#minValue
...
**继承:** THING.Streamline#maxValue

**重写:** THING.Streamline#maxValue
...
**继承:** THING.Streamline#extent

**重写:** THING.Streamline#extent

**描述:** 场数据范围 格式{minX:0,minY:0,maxX:10,maxY:10,minZ:0,maxZ:10}
...
**继承:** THING.Streamline#useDataColor

**重写:** THING.Streamline#useDataColor

**描述:** renderType为Line时 是否使用该位置的标量值通过色带对线进行颜色渲染
...
**继承:** THING.Streamline#speed

**重写:** THING.Streamline#speed

**描述:** renderType为Line时 代表贴图流动速度 renderType为Streamline时 代表流线的流动速度 无单位
...
**继承:** THING.Streamline#imageUrl

**重写:** THING.Streamline#imageUrl

**描述:** renderType为Line时 代表贴图路径
...
**继承:** THING.Streamline#gridData

**重写:** THING.Streamline#gridData
...
**重写:** THING.Streamline#isDynamicLoad
...
**重写:** THING.Streamline#frustumCulled
...
**重写:** THING.Streamline#renderOrder
...
**重写:** THING.Streamline#alwaysOnTop
...
**重写:** THING.Streamline#boundingBox
...
**重写:** THING.Streamline#orientedBox
...
**重写:** THING.Streamline#localPosition
...
**重写:** THING.Streamline#localAngles
...
**重写:** THING.Streamline#localRotation
...
**重写:** THING.Streamline#localQuaternion
...
**重写:** THING.Streamline#localScale
...
**重写:** THING.Streamline#quaternion
...
**重写:** THING.Streamline#ignoreParentTransform
...
**重写:** THING.Streamline#destroyable
...
**重写:** THING.Streamline#relationships
...
**重写:** THING.Streamline#components
...
**继承:** THING.Streamline#playLineGrow

**重写:** THING.Streamline#playLineGrow
...
|------|------|-------------|
| `param` | `Object` | 参数列表 |
| `param.speed` | `Number` | 生长速度 |
| `param.loopType` | `THING.LoopType` | 循环方式 |
...
**继承:** THING.Streamline#stopPlayLineGrow

**重写:** THING.Streamline#stopPlayLineGrow
...
**重写:** THING.Streamline#getVisible
...
**重写:** THING.Streamline#setVisible
...
**重写:** THING.Streamline#setPickable
...
**重写:** THING.Streamline#getFrustumCulled
...
**重写:** THING.Streamline#setFrustumCulled
...
**重写:** THING.Streamline#distanceTo
...
**重写:** THING.Streamline#promoteNode
...
**重写:** THING.Streamline#waitForComplete
...
**重写:** THING.Streamline#loadResource
...
**重写:** THING.Streamline#unloadResource
...
**重写:** THING.Streamline#getRenderOrder
...
**重写:** THING.Streamline#setRenderOrder
...
**重写:** THING.Streamline#makeInstancedDrawing
...
**重写:** THING.Streamline#enableInstancing
...
**重写:** THING.Streamline#disableInstancing
...
**重写:** THING.Streamline#setInstancing
...
**重写:** THING.Streamline#getInstancing
...
**重写:** THING.Streamline#setInstancingColor
...
**重写:** THING.Streamline#setInstancingClearOutColor
...
**重写:** THING.Streamline#getInstancingClearOutColor
...
**重写:** THING.Streamline#setInstancingDiffuseUVTransform
...
**重写:** THING.Streamline#getInstancingDiffuseUVTransform
...
**重写:** THING.Streamline#setInstancingVisible
...
**重写:** THING.Streamline#getInstancingVisible
...
**重写:** THING.Streamline#getInstancedCount
...
**重写:** THING.Streamline#setInstancingMatrix
...
**重写:** THING.Streamline#getInstancingMatrix
...
**重写:** THING.Streamline#setInstancingAttribute
...
**重写:** THING.Streamline#getInstancingAttribute
...
**重写:** THING.Streamline#stopMoving
...
**重写:** THING.Streamline#pauseMoving
...
**重写:** THING.Streamline#resumeMoving
...
**重写:** THING.Streamline#movePathAsync
...
**重写:** THING.Streamline#stopScaling
...
**重写:** THING.Streamline#pauseScaling
...
**重写:** THING.Streamline#resumeScaling
...
**重写:** THING.Streamline#stopRotating
...
**重写:** THING.Streamline#pauseRotating
...
**重写:** THING.Streamline#resumeRotating
...
**重写:** THING.Streamline#stopFading
...
**重写:** THING.Streamline#pauseFading
...
**重写:** THING.Streamline#resumeFading
...
**重写:** THING.Streamline#fadeInAsync
...
**重写:** THING.Streamline#fadeOutAsync
...
**重写:** THING.Streamline#stopFlying
...
**重写:** THING.Streamline#pauseFlying
...
**重写:** THING.Streamline#resumeFlying
...
**重写:** THING.Streamline#stopUVTransform
...
**重写:** THING.Streamline#pauseUVTransform
...
**重写:** THING.Streamline#resumeUVTransform
...
**重写:** THING.Streamline#uvTransformTo
...
**重写:** THING.Streamline#rotateOnAxis
...
**重写:** THING.Streamline#translateX
...
**重写:** THING.Streamline#translateY
...
**重写:** THING.Streamline#translateZ
...
**重写:** THING.Streamline#worldToSelf
...
**重写:** THING.Streamline#selfToWorld
...
**重写:** THING.Streamline#localToWorld
...
**重写:** THING.Streamline#worldToLocal
...
**重写:** THING.Streamline#hasAttribute
...
**重写:** THING.Streamline#getAttribute
...
**重写:** THING.Streamline#setAttribute
...
**重写:** THING.Streamline#setAttributes
...
**重写:** THING.Streamline#pauseEvent
...
**重写:** THING.Streamline#resumeEvent
...
**重写:** THING.Streamline#queryByName
...
**重写:** THING.Streamline#queryByTags
...
**重写:** THING.Streamline#queryByUUID
...
**重写:** THING.Streamline#queryByType
...
**重写:** THING.Streamline#queryByDTType
...
**重写:** THING.Streamline#queryByUserData
...
**重写:** THING.Streamline#queryByRegExp
...
**重写:** THING.Streamline#addComponent
...
**重写:** THING.Streamline#removeComponent
...
**重写:** THING.Streamline#removeAllComponents
...
**重写:** THING.Streamline#traverseComponentByType
...
**重写:** THING.Streamline#getComponentByName
...
**重写:** THING.Streamline#getComponentByType
...
**重写:** THING.Streamline#getComponentsByType
...
**重写:** THING.Streamline#getAllComponents
...
**重写:** THING.Streamline#hasComponent
