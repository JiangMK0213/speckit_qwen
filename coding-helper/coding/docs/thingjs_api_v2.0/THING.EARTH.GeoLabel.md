...
|------|------|--------|-------------|--------|
| param | `Object` | - | 否 | 参数列表 |
| param.coordinates | `Array.<Number>` | - | 是 | 地理位置坐标 一版GeoLabel会作为GeoObject的孩子 因此可以不传coordinates |
...
| param.style | `THING.EARTH.GeoLabelStyle` | - | 否 | 样式效果 |
| param.visible | `Boolean` | `true` | 是 | 是否显示 |
| param.onComplete | `function` | - | 是 | 地理对象创建完毕后的回调 |
| param.parent | `GeoObject` | - | 是 | 父物体 一般都是GeoObject |
