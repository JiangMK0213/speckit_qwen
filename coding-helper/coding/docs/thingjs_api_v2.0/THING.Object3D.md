...
| param.userData | `Array.<number>` | `''` | 是 | 对象name |
| param.localPosition | `Array.<number>` | `[0,0,0]` | 是 | 相对父物体的位置 |
| param.localScale | `Array.<number>` | `[1,1,1]` | 是 | 相对父物体的缩放 |
| param.localAngels | `Array.<number>` | `[0,0,0]` | 是 | 相对父物体的旋转角度 |
| param.position | `Array.<number>` | `[0,0,0]` | 是 | 世界坐标系下的位置 |
| param.scale | `Array.<number>` | `[1,1,1]` | 是 | 世界坐标系下的缩放 |
| param.angels | `Array.<number>` | `[0,0,0]` | 是 | T世界坐标系下的旋转角度 |
| param.pickable | `boolean` | `true` | 是 | 是否可拾取 |
| param.visible | `boolean` | `true` | 是 | 是否可见 详见visible的属性说明 |
| param.active | `boolean` | `true` | 是 | 是否激活 详见active的属性说明 |
| param.keepSize | `boolean` | - | 是 | 是否保持像素级别大小不变 |
| param.pivot | `Array.<number>` | - | 是 | 轴心点在自身包围盒中的位置(数组每一个分量取值范围0-1) |
| param.style | `object` | - | 是 | 样式设置 |
| param.castShadow | `boolean` | - | 是 | 是否投射阴影 |
| param.receiveShadow | `boolean` | - | 是 | 是否接收阴影 |
| param.alwaysOnTop | `boolean` | - | 是 | 是否始终在顶部 |
| param.onError | `function` | - | 是 | 创建失败的回调 |
| param.onComplete | `function` | - | 是 | 创建成功的回调 |
| param.onSyncComplete | `function` | - | 是 | 同步创建成功的回调 |
| param.onSyncBeforeDestroy | `function` | - | 是 | 同步销毁前的回调 |
