...
**数据类型:** `THING.Selector.<THING.Entity>`
...
**数据类型:** `THING.Selector.<THING.Floor>`
...
```javascript
let placements = building.placements;
```
...
| 名称 | 类型 | 描述 |
|------|------|-------------|
| `show` | `Boolean` | true 显示（默认） / false 隐藏 |
...
|------|------|-------------|
| `params` | `Object` | - |
| `params.duration` | `Number` | 展开时间（毫秒） |
| `params.distance` | `Number` | 展开间隔（米） |
| `params.horzMode` | `Boolean` | 展开方向，默认为垂直展开（false） |
| `params.onComplete` | `function` | 展开完成后的回调函数 |
| `params.onUpdate` | `function` | 展开过程中的回调函数 |

#### 示例

```javascript
building.expandFloors({	duration: 1000,	onUpdate: function (e) {		const floor = e.floor;		const progress = e.progress;		console.log(floor, progress);	},	onComplete: function () {		console.log('展开完成');	}});
```
...
|------|------|-------------|
| `params` | `Object` | - |
| `params.duration` | `Number` | 折叠时间（毫秒） |
| `params.onComplete` | `function` | 折叠完成后的回调函数 |
