...
|------|------|--------|-------------|--------|
| param | `Object` | - | 否 | - |
| param.id | `String` | - | 否 | 图层ID |
| param.name | `String` | - | 否 | 图层名称 |
| options.url | `String` | - | 是 | 数据源的地址 如果设置url会优先请求数据 如果请求失败则再去找dataSource属性,仅支持点类型的geojson数据 |
| param.dataSource | `GeoJSONFeatureCollection` | - | 否 | 数据源 |
| param.useCluster | `Boolean` | `true` | 是 | 是否聚合显示结果 默认true,如果传false,则在点数据的每一个位置根据属性创建一个柱子 |
| param.style | `THING.EARTH.GridLayerStyle` | - | 否 | 图层样式 |
| param.gridSize | `Number` | - | 否 | 网格大小 做聚合计算时使用 值越小运算速度越慢 单位:米 |
| param.shapeSize | `Number` | - | 否 | 展示结果使用的形状(方形柱,圆柱,圆锥等)的尺寸大小 单位:米 |
| param.vectorType | `String` | - | 否 | 展示结果使用的形状 支持box 方形柱 cylinder 圆柱 cone 圆锥 |
| param.extrudeField | `String` | - | 否 | 柱子高度字段 详见属性说明 |
| param.extrudeFieldArray | `Array.<String>` | - | 否 | 柱子需要分段展示时 每段柱子的高度字段 useCluster为false时生效 详见属性说明 |
| param.extrudeFactor | `Number` | `1` | 是 | 展示结果拔高系数 默认1 在运算结果的基础上乘以该系数代表柱子实际高度 |
| param.offsetHeightField | `String` | - | 否 | useCluster为false时 代表每个柱子的离地高度使用的字段 |
| param.offsetHeightFactor | `Number` | `1` | 是 | useCluster为false时 代表每个柱子的离地高度字段中取到的值的基础上再乘以一个系数作为最终离地高度 |
...
**描述:** 网格大小 做聚合计算时使用 值越小网格个数越大,运算速度越慢 单位:米
...
**数据类型:** `THING.EARTH.GridLayerVectorType`

**默认值:** `THING.EARTH.GridLayerVectorType.Box`

**描述:** 展示结果使用的形状 支持Box 方形柱 Cylinder 圆柱 Cone 圆锥
...
**描述:** 展示结果使用的形状(方形柱,圆柱,圆锥等)的尺寸大小 单位:米
