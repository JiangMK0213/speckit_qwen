...
|------|------|--------|-------------|--------|
| param | `Object` | - | 否 | - |
| param.lineType | `LineType` | `Plane` | 是 | 地理线类型，包括 Route，Pipe，Plane，Line |
...
| param.textureWrap | `TextureWrapMode` | `Stretch` | 是 | 贴图模式，包括 Repeat，Stretch |
| param.textureSize | `Number` | `100` | 是 | 纹理尺寸 代表线路方向的纹理尺寸 单位米 textureWrap=Repeat时生效 |
...
| param.effect | `Boolean` | `false` | 是 | 发光效果是否开启 |
| param.speed | `Number` | `0` | 是 | 流动速度 |
| param.sideType | `THING.SideType` | `THING.SideType.Front` | 是 | 正面/背面/双面渲染 注意:管线尽量使用Front,否则效果会有问题 |
...
**描述:** 获取/设置线型，例如，Line，Route，Plane，Pipe
...
**默认值:** `THING.EARTH.TextureWrapMode.Stretch`

**描述:** 获取/设置贴图模式 支持Stretch和Repeat
