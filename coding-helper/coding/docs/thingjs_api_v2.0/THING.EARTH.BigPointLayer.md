...
|------|------|--------|-------------|--------|
| param | `Object` | - | 否 | 参数列表 |
| param.style | `Object` | - | 否 | 海量点渲染样式 详见style属性的描述文档 |
| param.url | `String` | - | 是 | 图层数据源的url |
| param.offsetHeight | `Number` | - | 否 | 离地高度 |
| param.offsetHeightField | `String` | - | 否 | 离地高度字段 |
| param.offsetHeightFactor | `Number` | `1` | 是 | 离地高度倍数 |
| param.dataSource | `GeoJSONFeatureCollection` | - | 否 | 数据源 目前只支持Point格式的GeoJSON数据 |
...
**描述:** 海量点渲染样式 可参考THING.EARTH.GeoPointStyle需要注意的是其中的pointType仅支持Vecotr和Image,rotateSpeed与useSpriteMaterial不支持设置，另外可添加alphaMap对象控制整体透明效果。
...
**重写:** THING.EARTH.BigDataLayer#destroy
