...
**继承:** [THING.Object3D](./THING.Object3D.md)
...
|------|------|--------|-------------|--------|
| param | `Object` | - | 否 | 参数列表 |
| param.terrain | `Terrain` | - | 是 | 地形对象 |
| param.sceneMode | `THING.EARTH.SceneMode` | `THING.EARTH.SceneMode.EarthView` | 是 | 地图场景类型 三维地球 or 2.5维平面地图 |
| param.projectionType | `THING.EARTH.MapProjectionType` | `THING.EARTH.MapProjectionType.Geographic` | 是 | 2.5维平面地图时 创建平面使用的投影类型 经纬度直投 or Web墨卡托投影 |
| param.style | `THING.EARTH.MapStyle` | - | 是 | 地图样式 主要是指大气 雾效等地球级别的样式 数据结构可参考THING.EARTH.MapStyle |
| param.baseMaps | `Object, Array.<Object>` | - | 是 | 瓦片配置 数据结构可参考THING.EARTH.TileLayer |
| param.lights | `Object, Array.<Object>` | - | 是 | 灯光配置 数据结构可参考THING.EARTH.MapLightGroup |
| param.postEffects | `Object, Array.<Object>` | - | 是 | 后期配置 数据结构可参考THING.EARTH.MapPostEffects |
| param.background | `Object, Array.<Object>` | - | 是 | 背景配置 数据结构可参考THING.EARTH.MapBackground |
| param.restrictedLevel | `Array.<Number>` | - | 是 | 地图最大最小缩放级别 默认全级别显示 |
| param.attribution | `String` | - | 是 | 右下角地图的版权信息 |
| param.minPitch | `Number` | `8` | 是 | 相机最小倾角 |
| param.maxPitch | `Number` | `89.9` | 是 | 相机最大倾角 |
| param.onComplete | `function` | - | 是 | 创建完成的回调函数 |
...
**数据类型:** `MapRenderSettings`

**描述:** 地图环境配置 包括灯光 后期 背景等
...
**重写:** THING.Object3D#style

**描述:** 获取地图样式 如需要修改可以使用map.style.xxx = value;
...
**重写:** THING.Object3D#active

**描述:** 设置地图的显示/隐藏状态 不会影响图层的显示隐藏状态
...
**描述:** 设置深度模式0 (DepthMode.None) 不使用地球深度1 (DepthMode.DepthGlobe)采用深度球,深度球的半径要比地球半径小一些,这样地球背面的东西会被遮挡 但地球上的面和线仍然会显示 默认采用这种模式2 (DepthMode.Earth)采用地球瓦片真实深度,这样可能地球上直连的线和大面会被遮挡或者撕面,但是如果有地形或者有半地下的场景,需要采用这种模式应用时可以根据应用场景切换深度模式
...
**数据类型:** `THING.EARTH.SceneMode`

**默认值:** `THING.EARTH.SceneMode.EarthView`

**描述:** 地图场景类型 三维地球 or 2.5维平面地图
...
**数据类型:** `THING.EARTH.MapProjectionType`

**默认值:** `THING.EARTH.MapProjectionType.Geographic`

**描述:** 2.5维平面地图时 创建平面使用的投影类型 经纬度直投 or Web墨卡托投影

---

### useDepthStencilRenderBufferWhenUseMSAA
...
**描述:** 当开启MSAA时，是否使用深度模板作为帧缓冲区的附件，默认为false，只有深度附件注意：如果开启MSAA且加载贴地面和一部分倾斜摄影数据时，该参数应设置为true，否则效果会有问题
...
**重写:** THING.Object3D#isDynamicLoad
...
**重写:** THING.Object3D#frustumCulled
...
**重写:** THING.Object3D#localPosition
...
**重写:** THING.Object3D#localRotation
...
**重写:** THING.Object3D#localQuaternion
...
**重写:** THING.Object3D#ignoreParentTransform
...
**重写:** THING.Object3D#relationships
...
| `param.speed` | `Number` | 旋转速度 单位:(度/秒) |
| `param.complete` | `function` | 旋转完毕后的回调函数 |
...
**重写:** THING.Object3D#destroy

**描述:** 销毁地图时，会卸载所有图层，卸载大气，修改相机锚点矩阵为单位矩阵，同时也会卸载所有注册事件
...
**描述:** 切换到3D地球视图 注意:切换时地球上创建的物体不会跟着切换,需要上层控制隐藏或销毁
...
**重写:** THING.Object3D#getFrustumCulled
...
**重写:** THING.Object3D#setFrustumCulled
...
**重写:** THING.Object3D#waitForComplete
...
**重写:** THING.Object3D#loadResource
...
**重写:** THING.Object3D#unloadResource
...
**重写:** THING.Object3D#getRenderOrder
...
**重写:** THING.Object3D#setRenderOrder
...
**重写:** THING.Object3D#makeInstancedDrawing
...
**重写:** THING.Object3D#enableInstancing
...
**重写:** THING.Object3D#disableInstancing
...
**重写:** THING.Object3D#setInstancing
...
**重写:** THING.Object3D#getInstancing
...
**重写:** THING.Object3D#setInstancingColor
...
**重写:** THING.Object3D#setInstancingClearOutColor
...
**重写:** THING.Object3D#getInstancingClearOutColor
...
**重写:** THING.Object3D#setInstancingDiffuseUVTransform
...
**重写:** THING.Object3D#getInstancingDiffuseUVTransform
...
**重写:** THING.Object3D#setInstancingVisible
...
**重写:** THING.Object3D#getInstancingVisible
...
**重写:** THING.Object3D#getInstancedCount
...
**重写:** THING.Object3D#setInstancingMatrix
...
**重写:** THING.Object3D#getInstancingMatrix
...
**重写:** THING.Object3D#setInstancingAttribute
...
**重写:** THING.Object3D#getInstancingAttribute
...
**重写:** THING.Object3D#resumeMoving
...
**重写:** THING.Object3D#movePathAsync
...
**重写:** THING.Object3D#pauseScaling
...
**重写:** THING.Object3D#resumeScaling
...
**重写:** THING.Object3D#stopRotating
...
**重写:** THING.Object3D#pauseRotating
...
**重写:** THING.Object3D#resumeRotating
...
**重写:** THING.Object3D#resumeFading
...
**重写:** THING.Object3D#fadeOutAsync
...
**重写:** THING.Object3D#resumeFlying
...
**重写:** THING.Object3D#stopUVTransform
...
**重写:** THING.Object3D#pauseUVTransform
...
**重写:** THING.Object3D#resumeUVTransform
...
**重写:** THING.Object3D#uvTransformTo
...
**重写:** THING.Object3D#rotateOnAxis
...
**重写:** THING.Object3D#localToWorld
...
**重写:** THING.Object3D#worldToLocal
...
**重写:** THING.Object3D#hasAttribute
...
**重写:** THING.Object3D#getAttribute
...
**重写:** THING.Object3D#setAttribute
...
**重写:** THING.Object3D#setAttributes
...
**重写:** THING.Object3D#queryByDTType
...
**重写:** THING.Object3D#queryByUserData
...
**重写:** THING.Object3D#queryByRegExp
...
**重写:** THING.Object3D#addComponent
...
**重写:** THING.Object3D#removeComponent
...
**重写:** THING.Object3D#removeAllComponents
...
**重写:** THING.Object3D#traverseComponentByType
...
**重写:** THING.Object3D#getComponentByName
...
**重写:** THING.Object3D#getComponentByType
...
**重写:** THING.Object3D#getComponentsByType
...
**重写:** THING.Object3D#getAllComponents
...
**重写:** THING.Object3D#hasComponent
