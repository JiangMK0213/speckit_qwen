...
**描述:** 构造函数，用于初始化 Projector 对象。

**参数:**

| 名称 | 类型 | 默认值 | 可选 | 描述 |
|------|------|--------|-------------|--------|
| param | `object` | - | 否 | 初始化参数对象，包含 Projector 的配置选项。 |
| param.camera | `object` | - | 否 | 相机对象，用于渲染。 |
| param.receiveObjects | `Array` | - | 否 | 接收物体列表，逗号分隔。列表内元素支持uuid和Object对象。 |
| param.color | `string` | - | 否 | Projector 的颜色。 |
| param.opacity | `number` | - | 否 | Projector 的不透明度。 |
| param.debug | `boolean` | - | 否 | 是否启用调试模式。 |
| param.fov | `number` | - | 否 | 视场角。 |
| param.far | `number` | - | 否 | 远裁剪面。 |
| param.near | `number` | - | 否 | 近裁剪面。 |
| param.aspect | `number` | - | 否 | 纵横比。 |
| param.orthoSize | `number` | - | 否 | 正交投影大小。 |
| param.occlusion | `boolean` | - | 否 | 是否启用遮挡效果。 |
| param.occlusionBias | `number` | - | 否 | 遮挡偏差。 |
| param.occlusionQuality | `number` | - | 否 | 遮挡质量。 |
| param.fisheye | `string` | - | 否 | 鱼眼效果类型。 |
| param.barrelCorrection | `boolean` | - | 否 | 是否启用桶形畸变校正。 |
| param.barrelFx | `number` | - | 否 | 桶形畸变的 X 方向参数。 |
| param.barrelFy | `number` | - | 否 | 桶形畸变的 Y 方向参数。 |
| param.barrelS | `number` | - | 否 | 桶形畸变的缩放参数。 |
| param.projectionType | `string` | - | 否 | 投影类型。 |
| param.image | `object` | - | 否 | 图像参数，支持THING.ImageTexture和THING.VideoTexture两种THINGJS对象；同时支持结构为{type,url}的数据对象，数据对象参数如下。 |
| param.image.type | `object` | - | 否 | 图像类型<string>，支持'ImageTexture' 和 'VideoTexture'。 |
| param.image.url | `string` | - | 否 | 图像URL<string>。 |
...
**描述:** 设置/获取贴花相机视角（度）。使用此选项时相机类型为 Perspective。
...
**描述:** 设置/获取贴花相机正交尺寸。使用此选项时相机类型为 Orthographic。
...
**描述:** 设置/获取遮挡质量，默认值为 512。可选值包括 256、512、1024、2048 和 4096。
