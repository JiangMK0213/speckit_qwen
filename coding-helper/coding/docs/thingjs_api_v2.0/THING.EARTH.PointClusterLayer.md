...
| param.dataSource | `Array.<GeoJSON>` | - | 是 | 数据源 仅支持geojson的点数据 |
| param.url | `String` | - | 是 | 数据源url 接口通过GET方式请求的结果是geojson点数据的格式 优先级低于dataSource |
| param.style | `THING.EARTH.PointClusterLayerStyle` | - | 否 | 聚合图层渲染样式 |
| param.customRenderer | `function` | - | 是 | 自定义渲染方法 |
| param.pixelRange | `Number` | `100` | 是 | 扩展屏幕空间边界框的像素范围 单位像素 默认100 |
| param.minimumClusterSize | `Number` | `2` | 是 | 可以聚合的最小对象个数 默认2。 |
| param.onChange | `function` | - | 是 | 每次聚合结果改变时会触发该事件 初始化传入时，产生第一次聚合结果时就会触发。如果是创建之后监听，产生第一次聚合结果不会触发onChange事件。 |
...
**描述:** 每次聚合结果改变时会触发该事件初始化传入时，产生第一次聚合结果时就会触发。如果是创建之后监听，产生第一次聚合结果不会触发onChange事件。
