...
|------|------|--------|-------------|--------|
| param | `Object` | - | 否 | - |
| param.coordinates | `Array` | - | 否 | 地理位置坐标 |
| param.heights | `Array` | - | 是 | 线每个顶点抬高数组 数组长度与coordinates数组长度需一致 |
...
| param.offsetHeightField | `String` | - | 是 | 离地高度字段 如果设置了离地高度字段 物体最终的离地高度为offsetHeight与物体的userData[offsetHeightField]的值的总和 |
| param.offsetHeightFactor | `Number` | `1` | 是 | 离地高度字段倍数 |
| param.label | `THING.EARTH.GeoLabel` | - | 是 | 标注 |
| param.infoWindow | `THING.EARTH.InfoWindow` | - | 是 | 信息框 |
| param.visible | `Boolean` | `true` | 是 | 是否显示 |
| param.style | `THING.EARTH.GeoLineStyle` | - | 否 | 样式效果 |
