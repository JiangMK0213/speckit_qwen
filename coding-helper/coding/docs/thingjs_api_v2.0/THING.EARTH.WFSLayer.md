...
|------|------|--------|-------------|--------|
| param | `Object` | - | 否 | 参数列表 |
| param.name | `String` | - | 是 | 图层name |
| param.url | `String` | - | 否 | wfs服务的url |
| param.typeName | `String` | - | 否 | 加载数据过程中执行GetFeature请求时，指定要查询的地理数据集的名称 |
| param.version | `String` | `'1.0.0'` | 是 | wfs服务版本号 主要有1.0.0/1.1.0/2.0.0/2.0.2/3.0.0 |
| param.dynamicLoad | `Boolean` | `true` | 是 | 是否动态加载 |
| param.bbox | `Array.<Number>` | - | 是 | 动态加载为false时，可指定请求数据的经纬度范围 格式为[minLon,minLat,maxLon,maxLat] |
| param.maxFeatures | `Number` | - | 是 | 返回最大要素个数 |
| param.fetchLevel | `Number` | - | 是 | dynamicLoad为true时 可指定按照多少层级的瓦片大小进行动态请求 如果调度返回的瓦片层级小于fetchLevel,则不请求数据,如果调度返回的瓦片层级大于等于fetchLevel，则按照第fetchLevel层级的瓦片大小请求数据。如果不传，则按照调度返回的瓦片层级动态请求数据 |
| param.visibleLevel | `Array.<Number>` | - | 是 | 图层可见层级范围 |
| param.geoObjectType | `String` | - | 是 | 与FeatureLayer的geoObjectType含义一致 指定要创建的对象类型 |
| param.layerType | `String` | - | 是 | 最终创建图层的类型 详见属性说明 |
| param.style | `Object` | - | 是 | 图层样式 与创建FeatureLayer或BigDataLayer的样式一致 |
...
**描述:** 强制刷新WFSLayer,根据最新的相机信息更新图层内容
