...
| 名称 | 类型 | 默认值 | 可选 | 描述 |
|------|------|--------|-------------|--------|
| param | `object` | - | 否 | 初始化参数,除Object3D基础的初始化参数以外的参数 |
| param.autoFitBodyScale | `boolean` | `false` | 是 | 是否自适应body的缩放 |
| param.scaleFactor | `number` | `0.01` | 是 | 缩放系数 |
| param.renderType | `THING.RenderType` | `THING.RenderType.Sprite` | 是 | 渲染类型 平面或精灵 |
| param.spriteRotation | `number` | `0` | 是 | 精灵模式下的旋转角度 |
