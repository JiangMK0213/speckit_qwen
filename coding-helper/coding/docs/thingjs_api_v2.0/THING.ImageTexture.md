...
**描述:** 图像纹理，用于加载图像资源并在样式中使用。

**参数:**

| 名称 | 类型 | 默认值 | 可选 | 描述 |
|------|------|--------|-------------|--------|
| param | `object, PixelBuffer, CanvasResource` | - | 否 | 初始化参数，支持 url、数据类型的纹理 Buffer 和 Canvas 类型的纹理 Buffer |
| param.url | `string` | - | 否 | 贴图资源的路径 |
...
**描述:** 是否生成 mipMap。如果开启，纹理的 minFilterType 会被自动改成 LinearMipmapLinear
