...
**描述:** 可自定义坐标的平面物体(所有点位均在同一平面)

**参数:**

| 名称 | 类型 | 默认值 | 可选 | 描述 |
|------|------|--------|-------------|--------|
| param | `object` | - | 否 | 初始参数列表 |
| param.points | `Array.<Array.<number>>` | - | 否 | 世界坐标系下(3D)区域外轮廓坐标 |
| param.holes | `Array.<Array.<Array.<number>>>` | - | 否 | 世界坐标系下(3D)区域内空洞的坐标(支持多个空洞) |
| param.selfPoints | `Array.<Array.<number>>` | - | 否 | 自身坐标系下(3D)区域外轮廓坐标 |
| param.selfHoles | `Array.<Array.<Array.<number>>>` | - | 否 | 自身坐标系下(3D)区域空洞坐标 |
| param.selfPlanePoints | `Array.<Array.<number>>` | - | 否 | 自身平面坐标系下(2D)区域外轮廓坐标 |
| param.selfPlaneHoles | `Array.<Array.<Array.<number>>>` | - | 否 | 自身平面坐标系下(2D)区域空洞坐标 |
