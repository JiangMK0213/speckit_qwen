...
|------|------|--------|-------------|--------|
| param | `Object` | - | 否 | 参数列表 |
| param.data | `Array` | - | 否 | 数据源 |
| param.clippingPlane | `Object` | - | 是 | 剖切面属性 |
| param.clippingPlane.normal | `Array` | - | 是 | 剖切面法线 如 [0,0,1] |
| param.clippingPlane.constant | `Number` | - | 是 | 剖切面constant值 |
| param.imageUrls | `Array` | - | 是 | 贴图图像数组url |
| param.colors | `Array` | - | 是 | 颜色数组 |
| param.imageRepeat | `Array` | - | 是 | 贴图重复度 |
| param.gridSize | `Number` | - | 是 | 地质表面三角化参数，值越小，生成三角形面积越小，生成三角形数量越多 如果不传会根据数据范围适配 |
| param.triangulationWasmUrl | `String` | - | 否 | 构造地质体三角面时需要使用的一个外部wasm库的地址 必填 |
...
**描述:** 获取/设置地层渲染使用的贴图路径 数组长度与地层个数一致

#### 参数

| 名称 | 类型 | 描述 |
|------|------|-------------|
| `imageUrls` | `Array.<String>` | 贴图路径数组 |
...
**描述:** 获取/设置地层渲染使用的颜色 数组长度与地层个数一致

#### 参数

| 名称 | 类型 | 描述 |
|------|------|-------------|
| `colorArray` | `Array` | 颜色数组 |
...
| `index` | `Boolean` | 地层索引 |
| `effectCylinder` | `Boolean` | 是否影响钻井柱子 |

---

### setTopBlockVisibleByIndex
