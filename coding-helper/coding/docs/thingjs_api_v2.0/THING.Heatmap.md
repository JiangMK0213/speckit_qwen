...
|------|------|--------|-------------|--------|
| param | `Object` | - | 否 | 参数列表 |
| param.renderType | `HeatmapRenderType` | `THING.HeatmapRenderType.Plane` | 是 | 是平面的热力图还是3d的热力图THING.HeatmapRenderType.Plane代表平面热力图 THING.HeatmapRenderType.Mesh代表3d热力图 |
| param.extent | `Number` | - | 是 | 热力图展示的范围,优先级最高 例如{minX:-50,minY:-50,maxX:50,maxY:50}  数值单位是米 代表绝对坐标 |
...
| param.minMaxRatio | `Array` | `[0,1]` | 是 | 剔除比例最小值 最大值, 默认[0,1]代表不剔除,大于最大值的值会被剔除,小于最小值的值会被剔除 |
| param.gradient | `Object` | `false` | 是 | 渲染热力图使用的色带 |
| param.maskShape | `Object3D` | - | 是 | 基于maskShape作为展示热力图的对象, 这里需要注意maskShape是一个xz平面的片,非地球模式下生效 |
| param.mask | `Array` | - | 是 | 渲染热力图使用的遮罩 格式为:{
			points: [[0, 0], [-10, 0], [-10, -10], [10, -10], [10, 10], [0, 10]],
			holes: [
					[[2, -1], [4, -1], [4, -3], [2, -3]]
				]
			} |
| param.mosaic | `Object` | - | 是 | 热力图是否使用马赛克效果 格式为{enable:true,size:2}
		enable代表是否使用马赛克 size代表马赛克尺寸 如果不在地图下使用 size代表一个马赛克的尺寸为多少米 如果在地图下使用 size代表一个马赛克尺寸是多少度 |
