# 类: EarthVolumeRendering

**命名空间:** THING

**继承:** [THING.VolumeRendering](./THING.VolumeRendering.md)
...
| 名称 | 类型 | 默认值 | 可选 | 描述 |
|------|------|--------|-------------|--------|
| param | `Object` | - | 否 | 参数列表 体渲染是数据源 url data textureUrl cubTextureData设置其中一个即可 |
| param.url | `String` | - | 是 | 点位数据的url 数据内容就与data参数是一致的 |
| param.data | `Array` | - | 是 | 计算使用的数据(优先级高于体纹理路径) 格式为[[x1,y1,z1,value1],[x2,y2,z2,value2]......] |
| param.textureUrl | `String` | - | 是 | 体纹理路径(多层拼成的一个图片,类似于ct胶片) 需要配合textureResolution使用 |
| param.cubeTextureData | `TypedArray` | - | 是 | 三维体纹理数据(归一化之后的标量值，长度为textureResolution三个值的乘积) 需要配合textureResolution使用 |
| param.textureResolution | `Array.<Number>` | - | 是 | 体纹理在三个维度的尺寸 例如[128,128,32] 代表每一层纹理是128*128像素 一共32层 |
| param.extent | `Object` | - | 是 | 如果在使用体纹理做体渲染 需要指定数据的范围  例如{minX:116,minY:39,maxX:117,maxY:40,minZ:0,maxZ:10000} 数值代表的是经纬度 x代表最小经度 y代表最小纬度 z代表高度 |
| param.alphaCorrection | `Number` | `1.0` | 是 | 透明度校正 |
| param.data | `Array` | - | 是 | 计算使用的数据(优先级高于体纹理路径) 格式为[[x1,y1,z1,value1],[x2,y2,z2,value2]......] xyz分别代表经度纬度高度 |
| param.gradient | `Object` | - | 是 | 渐变色 |
...
**继承:** THING.VolumeRendering#clippingPlane

**重写:** THING.VolumeRendering#clippingPlane

**描述:** 设置/获取 体渲染剖切面 格式为{normal:剖切面的法线方向,constant:距离原点距离}
...
**继承:** THING.VolumeRendering#textureUrl

**重写:** THING.VolumeRendering#textureUrl
...
**继承:** THING.VolumeRendering#textureResolution

**重写:** THING.VolumeRendering#textureResolution

**描述:** 体渲染分辨率 [x,y,z] x,y为体纹理宽高 目前x与y必须相等 z代表层数
...
**继承:** THING.VolumeRendering#extent

**重写:** THING.VolumeRendering#extent

**描述:** 体渲染数据范围 数据格式{minX:116,minY:39,minZ:0,maxX:117,maxY:40,maxZ:100}如果使用体纹理该参数是必须的 如果使用数据源会基于数据中的位置自动计算
...
**默认值:** `{ 0: 'rgb(0,0,255)', 0.33: 'rgb(0,255,0)', 0.66: 'rgb(255,255,0)', 1.0: 'rgb(255,0,0)' }`

**继承:** THING.VolumeRendering#gradient

**重写:** THING.VolumeRendering#gradient
...
```javascript
//色带对象的key为位置 值为对应的颜色值volumeRenderingLayer.gradient = { 0: 'rgb(0,0,255)', 0.33: 'rgb(0,255,0)', 0.66: 'rgb(255,255,0)', 1.0: 'rgb(255,0,0)'};
```
...
**继承:** THING.VolumeRendering#alphaCorrection

**重写:** THING.VolumeRendering#alphaCorrection

**描述:** 体渲染透明度校正 体渲染计算时将读取的透明度与alphaCoorection相乘,然后逐步累加透明度

#### 示例

```javascript
volumeRenderingLayer.alphaCorrection = 0.5;
```
...
**继承:** THING.VolumeRendering#data

**重写:** THING.VolumeRendering#data
...
**重写:** THING.VolumeRendering#helper
...
**重写:** THING.VolumeRendering#level
...
**重写:** THING.VolumeRendering#collider
...
**重写:** THING.VolumeRendering#isDynamicLoad
...
**重写:** THING.VolumeRendering#layerMask
...
**重写:** THING.VolumeRendering#styleTag
...
**重写:** THING.VolumeRendering#inherit
...
**重写:** THING.VolumeRendering#visible
...
**重写:** THING.VolumeRendering#pickable
...
**重写:** THING.VolumeRendering#active
...
**重写:** THING.VolumeRendering#frustumCulled
...
**重写:** THING.VolumeRendering#keepSize
...
**重写:** THING.VolumeRendering#pivot
...
**重写:** THING.VolumeRendering#style
...
**重写:** THING.VolumeRendering#renderOrder
...
**重写:** THING.VolumeRendering#alwaysOnTop
...
**重写:** THING.VolumeRendering#boundingBox
...
**重写:** THING.VolumeRendering#orientedBox
...
**重写:** THING.VolumeRendering#isFlying
...
**重写:** THING.VolumeRendering#localPosition
...
**重写:** THING.VolumeRendering#localAngles
...
**重写:** THING.VolumeRendering#localRotation
...
**重写:** THING.VolumeRendering#localQuaternion
...
**重写:** THING.VolumeRendering#localScale
...
**重写:** THING.VolumeRendering#position
...
**重写:** THING.VolumeRendering#angles
...
**重写:** THING.VolumeRendering#rotation
...
**重写:** THING.VolumeRendering#quaternion
...
**重写:** THING.VolumeRendering#scale
...
**重写:** THING.VolumeRendering#ignoreParentTransform
...
**重写:** THING.VolumeRendering#dtType
...
**重写:** THING.VolumeRendering#userData
...
**重写:** THING.VolumeRendering#destroyable
...
**重写:** THING.VolumeRendering#queryable
...
**重写:** THING.VolumeRendering#parent
...
**重写:** THING.VolumeRendering#relationships
...
**重写:** THING.VolumeRendering#parents
...
**重写:** THING.VolumeRendering#brothers
...
**重写:** THING.VolumeRendering#children
...
**重写:** THING.VolumeRendering#components
...
**重写:** THING.VolumeRendering#getVisible
...
**重写:** THING.VolumeRendering#setVisible
...
**重写:** THING.VolumeRendering#setPickable
...
**重写:** THING.VolumeRendering#getFrustumCulled
...
**重写:** THING.VolumeRendering#setFrustumCulled
...
**重写:** THING.VolumeRendering#distanceTo
...
**重写:** THING.VolumeRendering#promoteNode
...
**重写:** THING.VolumeRendering#raycast
...
**重写:** THING.VolumeRendering#setStyle
...
**重写:** THING.VolumeRendering#clone
...
**重写:** THING.VolumeRendering#waitForComplete
...
**重写:** THING.VolumeRendering#loadResource
...
**重写:** THING.VolumeRendering#unloadResource
...
**重写:** THING.VolumeRendering#getRenderOrder
...
**重写:** THING.VolumeRendering#setRenderOrder
...
**重写:** THING.VolumeRendering#makeInstancedDrawing
...
**重写:** THING.VolumeRendering#enableInstancing
...
**重写:** THING.VolumeRendering#disableInstancing
...
**重写:** THING.VolumeRendering#setInstancing
...
**重写:** THING.VolumeRendering#getInstancing
...
**重写:** THING.VolumeRendering#setInstancingColor
...
**重写:** THING.VolumeRendering#setInstancingClearOutColor
...
**重写:** THING.VolumeRendering#getInstancingClearOutColor
...
**重写:** THING.VolumeRendering#setInstancingDiffuseUVTransform
...
**重写:** THING.VolumeRendering#getInstancingDiffuseUVTransform
...
**重写:** THING.VolumeRendering#setInstancingVisible
...
**重写:** THING.VolumeRendering#getInstancingVisible
...
**重写:** THING.VolumeRendering#getInstancedCount
...
**重写:** THING.VolumeRendering#setInstancingMatrix
...
**重写:** THING.VolumeRendering#getInstancingMatrix
...
**重写:** THING.VolumeRendering#setInstancingAttribute
...
**重写:** THING.VolumeRendering#getInstancingAttribute
...
**重写:** THING.VolumeRendering#getAABB
...
**重写:** THING.VolumeRendering#getOBB
...
**重写:** THING.VolumeRendering#stopMoving
...
**重写:** THING.VolumeRendering#pauseMoving
...
**重写:** THING.VolumeRendering#resumeMoving
...
**重写:** THING.VolumeRendering#moveTo
...
**重写:** THING.VolumeRendering#movePath
...
**重写:** THING.VolumeRendering#movePathAsync
...
**重写:** THING.VolumeRendering#stopScaling
...
**重写:** THING.VolumeRendering#pauseScaling
...
**重写:** THING.VolumeRendering#resumeScaling
...
**重写:** THING.VolumeRendering#scaleTo
...
**重写:** THING.VolumeRendering#stopRotating
...
**重写:** THING.VolumeRendering#pauseRotating
...
**重写:** THING.VolumeRendering#resumeRotating
...
**重写:** THING.VolumeRendering#rotateTo
...
**重写:** THING.VolumeRendering#stopFading
...
**重写:** THING.VolumeRendering#pauseFading
...
**重写:** THING.VolumeRendering#resumeFading
...
**重写:** THING.VolumeRendering#fadeIn
...
**重写:** THING.VolumeRendering#fadeOut
...
**重写:** THING.VolumeRendering#fadeInAsync
...
**重写:** THING.VolumeRendering#fadeOutAsync
...
**重写:** THING.VolumeRendering#stopFlying
...
**重写:** THING.VolumeRendering#pauseFlying
...
**重写:** THING.VolumeRendering#resumeFlying
...
**重写:** THING.VolumeRendering#flyTo
...
**重写:** THING.VolumeRendering#stopUVTransform
...
**重写:** THING.VolumeRendering#pauseUVTransform
...
**重写:** THING.VolumeRendering#resumeUVTransform
...
**重写:** THING.VolumeRendering#uvTransformTo
...
**重写:** THING.VolumeRendering#rotate
...
**重写:** THING.VolumeRendering#rotateOnAxis
...
**重写:** THING.VolumeRendering#rotateX
...
**重写:** THING.VolumeRendering#rotateY
...
**重写:** THING.VolumeRendering#rotateZ
...
**重写:** THING.VolumeRendering#translateX
...
**重写:** THING.VolumeRendering#translateY
...
**重写:** THING.VolumeRendering#translateZ
...
**重写:** THING.VolumeRendering#translate
...
**重写:** THING.VolumeRendering#lookAt
...
**重写:** THING.VolumeRendering#worldToSelf
...
**重写:** THING.VolumeRendering#selfToWorld
...
**重写:** THING.VolumeRendering#localToWorld
...
**重写:** THING.VolumeRendering#worldToLocal
...
**重写:** THING.VolumeRendering#hasAttribute
...
**重写:** THING.VolumeRendering#getAttribute
...
**重写:** THING.VolumeRendering#setAttribute
...
**重写:** THING.VolumeRendering#setAttributes
...
**重写:** THING.VolumeRendering#destroy
...
**重写:** THING.VolumeRendering#pauseEvent
...
**重写:** THING.VolumeRendering#resumeEvent
...
**重写:** THING.VolumeRendering#trigger
...
**重写:** THING.VolumeRendering#invoke
...
**重写:** THING.VolumeRendering#remove
...
**重写:** THING.VolumeRendering#traverse
...
**重写:** THING.VolumeRendering#query
...
**重写:** THING.VolumeRendering#queryAll
...
**重写:** THING.VolumeRendering#queryByName
...
**重写:** THING.VolumeRendering#queryById
...
**重写:** THING.VolumeRendering#queryByTags
...
**重写:** THING.VolumeRendering#queryByUUID
...
**重写:** THING.VolumeRendering#queryByType
...
**重写:** THING.VolumeRendering#queryByDTType
...
**重写:** THING.VolumeRendering#queryByUserData
...
**重写:** THING.VolumeRendering#queryByRegExp
...
**重写:** THING.VolumeRendering#addComponent
...
**重写:** THING.VolumeRendering#removeComponent
...
**重写:** THING.VolumeRendering#removeAllComponents
...
**重写:** THING.VolumeRendering#traverseComponentByType
...
**重写:** THING.VolumeRendering#getComponentByName
...
**重写:** THING.VolumeRendering#getComponentByType
...
**重写:** THING.VolumeRendering#getComponentsByType
...
**重写:** THING.VolumeRendering#getAllComponents
...
**重写:** THING.VolumeRendering#hasComponent
