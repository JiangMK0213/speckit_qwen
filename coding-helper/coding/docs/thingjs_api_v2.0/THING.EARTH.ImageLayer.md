...
|------|------|--------|-------------|--------|
| param | `Object` | - | 否 | - |
| param.useMercatorUV | `Boolean` | `false` | 是 | 贴图时是否采用墨卡托投影 这个取决于图片本身 |
| param.mask | `Array.<GeoJSON>` | - | 是 | 遮罩的坐标 支持多边形格式的GeoJSON数据 |
| param.extent | `Extent` | `{minX:-180, minY:-90, maxX:180, maxY:90}` | 是 | 贴图代表的地理范围 |
| param.style | `THING.EARTH.CanvasLayerStyle` | - | 否 | 图层样式 |
