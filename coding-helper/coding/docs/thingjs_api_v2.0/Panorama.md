...
|------|------|--------|-------------|--------|
| param | `object` | - | 否 | - |
| param.data | `Array.<Object>` | - | 否 | 全景图的所有数据 |
| param.firstIndex | `number` | - | 否 | 初始化时加载的全景图数据索引，默认为0 |
| param.onComplete | `function` | - | 否 | 创建全景图后的完成回调函数，默认为null |
...
**数据类型:** `THING.EXTEND.PanoSwitcher`
...
**数据类型:** `Array.<THING.EXTEND.PanoObejct>`
...
|------|------|-------------|
| `param` | `object` | - |
| `param.link` | `object` | 全景图数据，例如 { uri: './pano/uino/IMG_20220909_102355_00_003.jpg', rotationY: 0, position: [-30, 0, 0], link: [1] } |
| `param.time` | `number` | 全景图动画的时间 |
| `param.animationType` | `PanoramaAnimationType` | 全景图动画类型 |
| `param.onUpdate` | `function` | 播放全景图动画时的更新回调函数 |
| `param.onComplete` | `function` | 全景图动画完成后的回调函数 |
...
**描述:** 获取可以从指定全景图索引直接切换的所有全景图信息

#### 参数

| 名称 | 类型 | 描述 |
|------|------|-------------|
| `index` | `number` | 我们想要获取信息的全景图索引，默认为this.currentIndex |

#### 返回值

- **类型:** `Array.<Object>`
- **描述:** 例如 [{ from: 3, to: 2, direction: [-1,0,0] }]
